...
import { generateTryOnImage, generateMultipleTryOnImages, generateFashionVideo, hasApiKey } from './services/geminiService';
import { generateMultipleSeeDreamImages, generateMultipleFluxImages, hasWavespeedApiKey } from './services/seedreamService';
import { saveApiKeyToLocalStorage, clearApiKeyFromLocalStorage } from './services/configService';
...
const App: React.FC = () => {
  ...
  const handleSaveApiKey = () => {
    if (apiKeyInput.trim()) {
      saveApiKeyToLocalStorage('gemini', apiKeyInput.trim());
      setHasKey(true);
  ...
  const handleSaveWavespeedApiKey = () => {
    if (wavespeedApiKeyInput.trim()) {
      saveApiKeyToLocalStorage('wavespeed', wavespeedApiKeyInput.trim());
  ...
  const handleShare = async () => {
    ...
    const shareText = "-dime como me queda mi Look #sirena";
    
    try {
      // Convert data URL to blob/file for sharing
      const response = await fetch(result.imageUrl);
      const blob = await response.blob();
      const file = new File([blob], 'my-vogue-look.png', { type: 'image/png' });

      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          text: shareText,
          files: [file],
          title: 'My Vogue AI Look'
        });
      } else {
        // Fallback: Copy text and alert user
        await navigator.clipboard.writeText(shareText);
        alert('Texto copiado al portapapeles: "' + shareText + '". ¡Descarga la imagen y compártela!');
      ...
      setError("No se pudo compartir. Intenta descargar la imagen manualmente.");
    }
  };
  ...
  const handleDeleteAllClothing = () => {
    // Clear all sample clothes
    setSampleClothes([]);
    // Clear all custom clothes
    saveCustomClothes([]);
    ...
    <div className="min-h-screen bg-white text-stone-900 selection:bg-stone-900 selection:text-white pb-20">
      ...
      <main className="max-w-7xl mx-auto px-6 pt-32">
        ...
        <div className="flex justify-end mb-4 gap-4">
          <button
            onClick={reset}
            className="flex items-center gap-2 px-6 py-2 bg-stone-100 text-stone-600 hover:bg-stone-900 hover:text-white transition-all duration-300 text-[10px] uppercase tracking-widest font-bold border border-stone-200"
          >
        ...
        {/* WORKSPACE GRID */}
        <div className={`grid grid-cols-1 lg:grid-cols-12 gap-1 lg:h-[750px] border border-stone-200 bg-stone-200 mb-12 transition-all duration-700 ${status !== AppStatus.IDLE ? 'opacity-40 grayscale pointer-events-none' : ''}`}>
          
          {/* LEFT: THE MUSE (Larger) */}
          <div className="lg:col-span-8 bg-white h-[600px] lg:h-full">
            <UploadZone
              label="The Muse"
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                    ...
                    if (confirm('¿Eliminar TODA la ropa del wardrobe?')) {
                      handleDeleteAllClothing();
                    }
                  }}
                  ...
                  <div className="p-8 bg-white shadow-[0_30px_60px_-15px_rgba(0,0,0,0.15)] border border-stone-100 max-w-full mx-auto">
                    ...
                    <div className="pt-6 pb-2 flex justify-between items-end border-t border-stone-100 mt-6">
                        ...
                        <div className="flex gap-4 items-center">
                            <button 
                                onClick={handleShare}
                                className="text-xs font-bold uppercase tracking-widest text-blue-600 border-b border-blue-600 pb-1 hover:text-blue-800 hover:border-blue-800 transition-colors flex items-center gap-1"
                            >
